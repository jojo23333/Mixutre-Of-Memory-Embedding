...
def set_impl(impl):
    """Set the implementation override ('fa3', 'sdpa', or None for env/default)."""
...
class TestFA3VsSDPA:
    ...
    @pytest.mark.xfail(
        reason="FA3 backward differs on the current Hopper stack; SDPA is the supported default backend.",
        strict=False,
    )
    def test_backward_gradients_match(self):
        """Verify gradients are similar between FA3 and SDPA when FA3 is healthy."""
        B, T, H, D = 2, 32, 4, 16
...
class TestOverrideMechanism:
    ...
    def test_default_impl_is_sdpa(self, monkeypatch):
        """Default selection should stay on SDPA unless explicitly overridden."""
        monkeypatch.delenv("NANOCHAT_FLASH_IMPL", raising=False)
        set_impl(None)
        assert fa_module.configured_flash_impl() == "sdpa"
        assert fa_module.active_flash_impl() == "sdpa"
    ...
    def test_override_auto(self, monkeypatch):
        """Test that auto mode still follows hardware detection when requested."""
        monkeypatch.setenv("NANOCHAT_FLASH_IMPL", "auto")
        set_impl(None)
        assert fa_module._use_fa3() == HAS_FA3

    @pytest.mark.skipif(not HAS_FA3, reason="FA3 required")
    def test_env_fa3_opt_in(self, monkeypatch):
        """FA3 should only activate when explicitly requested."""
        monkeypatch.setenv("NANOCHAT_FLASH_IMPL", "fa3")
        set_impl(None)
        assert fa_module.active_flash_impl() == "fa3"
