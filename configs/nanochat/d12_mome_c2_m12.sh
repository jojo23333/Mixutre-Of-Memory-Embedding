...
exec "${PYTHON:-python}" -m scripts.run \
  ...
  --model-type gpt \
  --moe-ve \
  --moe-ve-setting 0_2_12 \
  --moe-ve-gate-nl sigmoid-norm \
  --moe-ve-router-input hidden \
  --moe-ve-slot-mapping table \
  --moe-ve-slot-factor 2 \
  --moe-ve-balance-lr 0 \
  ...
  --moe-ve-slot-map-path artifacts/slotmaps/t2_d24_dense_ve_6e19_20260425T075852Z_fixed_knn_table_slotmap_k2_step022412.json
